...
function RenderElement(props) {
  const $ = reactCompilerRuntime.c(45), editorActor = React.useContext(EditorActorContext), schema2 = react.useSelector(editorActor, _temp$2);
...
const debug$f = debugWithName("sync machine"), syncValueCallback = ({
  sendBack,
  input
}) => {
  updateValue({
  ...
  states: {
    "setting up": {
      ...
      states: {
        "syncing initial value": {
          entry: [() => {
            debug$f("entry: syncing initial value");
          }],
          exit: [() => {
            debug$f("exit: syncing initial value");
          }],
          always: {
        ...
        "done syncing initial value": {
          entry: ["emit done syncing value", () => {
            debug$f("entry: done syncing initial value");
          }],
          exit: [() => {
            debug$f("exit: done syncing initial value");
          }],
          type: "final"
        }
      }
    ...
    syncing: {
      ...
      states: {
        idle: {
          entry: [() => {
            debug$f("entry: syncing->idle");
          }],
          exit: [() => {
            debug$f("exit: syncing->idle");
          }],
          on: {
            "update value": [{
              guard: "is busy",
        ...
        syncing: {
          entry: [() => {
            debug$f("entry: syncing->syncing");
          }, "emit syncing value"],
          exit: [() => {
            debug$f("exit: syncing->syncing");
...
async function updateValue({
  ...
  if ((!value || value.length === 0) && (debug$f("Value is empty"), slate.Editor.withoutNormalizing(slateEditor, () => {
  ...
  if (!isValid) {
    debug$f("Invalid value, returning"), sendBack({
      type: "done syncing",
      value
    });
    return;
  ...
  if (isChanged) {
    debug$f("Server value changed, syncing editor");
    try {
    ...
    debug$f("Server value and editor value is equal, no need to sync.");
  sendBack({
    type: "done syncing",
    value
  });
    ...
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        if (hasChanges && blockValid) {
          ...
          })), validation.valid || validation.resolution?.autoResolve ? (oldBlock._key === currentBlock._key ? (debug$f.enabled && debug$f("Updating block", oldBlock, currentBlock), _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex)) : (debug$f.enabled && debug$f("Replacing block", oldBlock, currentBlock), _replaceBlock(slateEditor, currentBlock, currentBlockIndex)), blockChanged = !0) : (sendBack({
            type: "invalid value",
        ...
        if (!oldBlock && blockValid) {
          const validationValue = [value[currentBlockIndex]], validation = validateValue(validationValue, context.schema, context.keyGenerator);
          debug$f.enabled && debug$f("Validating and inserting new block in the end of the value", currentBlock), validation.valid || validation.resolution?.autoResolve ? slate.Transforms.insertNodes(slateEditor, currentBlock, {
            at: [currentBlockIndex]
          }) : (debug$f("Invalid", validation), sendBack({
            type: "invalid value",
            resolution: validation.resolution,
            value
          }), blockValid = !1);
...
function _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex) {
  if (slate.Transforms.setNodes(slateEditor, currentBlock, {
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach((_, index) => {
      ...
      childIndex > 0 && (debug$f("Removing child"), slate.Transforms.removeNodes(slateEditor, {
      ...
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key) {
          debug$f("Updating changed child", currentBlockChild, oldBlockChild), slate.Transforms.setNodes(slateEditor, currentBlockChild, {
            at: path
          });
          ...
          isSpanNode && isTextChanged ? (oldBlockChild.text.length > 0 && slate.Transforms.delete(slateEditor, {
            ...
            at: path
          }), slateEditor.onChange()) : isSpanNode || (debug$f("Updating changed inline object child", currentBlockChild), slate.Transforms.setNodes(slateEditor, {
            _key: VOID_CHILD_KEY
          }, {
            at: [...path, 0],
            voids: !0
          }));
        } else oldBlockChild ? (debug$f("Replacing child", currentBlockChild), slate.Transforms.removeNodes(slateEditor, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slate.Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange()) : oldBlockChild || (debug$f("Inserting new child", currentBlockChild), slate.Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange());
    });
  }
}
const debug$e = debugWithName("component:PortableTextEditor:Synchronizer");
function Synchronizer(props) {
  const $ = reactCompilerRuntime.c(41), {
    editorActor,
    slateEditor
  } = props, incomingValue = react.useSelector(props.editorActor, _temp$1), readOnly = react.useSelector(props.editorActor, _temp2);
  let t0;
  ...
  $[30] !== incomingValue || $[31] !== syncActorRef ? (t13 = () => {
    debug$e("Value from props changed, syncing new value"), syncActorRef.send({
      type: "update value",
      value: incomingValue
    });
  ...
  $[34] !== editorActor || $[35] !== mutationActorRef ? (t15 = () => {
    debug$e("Subscribing to patch events");
    ...
    return () => {
      debug$e("Unsubscribing to patch events"), sub.unsubscribe();
    };
...
const debug$d = debugWithName("operationToPatches");
function createOperationToPatches(editorActor) {
  ...
  function insertTextPatch(editor, operation, beforeValue) {
    debug$d.enabled && debug$d("Operation", JSON.stringify(operation, null, 2));
  ...
  function insertNodePatch(editor, operation, beforeValue) {
    ...
    return debug$d("Something was inserted into a void block. Not producing editor patches."), [];
  }
  ...
  function removeNodePatch(editor, operation, beforeValue) {
    ...
    if (operation.path.length === 1) {
      ...
      }])] : (debug$d("Span not found in editor trying to remove node"), []);
    } else
      return debug$d("Not creating patch inside object block"), [];
  }
  function mergeNodePatch(editor, operation, beforeValue) {
    ...
    if (operation.path.length === 1)
      ...
      debug$d("Void nodes can't be merged, not creating any patches");
    return patches$1;
  }
...
const debug$b = debugWithName("applyPatches"), debugVerbose = debug$b.enabled && !0;
function createApplyPatch(schema2) {
  return (editor, patch) => {
    let changed = !1;
    debugVerbose && (debug$b(`

NEW PATCH =============================================================`), debug$b(JSON.stringify(patch, null, 2)));
    try {
      switch (patch.type) {
        case "insert":
        ...
        case "diffMatchPatch":
          ...
          break;
        default:
          debug$b("Unhandled patch", patch.type);
      }
    } catch (err) {
      console.error(err);
    }
...
function diffMatchPatch(editor, patch) {
  ...
  if (!block)
    return debug$b("Block not found"), !1;
  if (!child || !childPath)
    return debug$b("Child not found"), !1;
...
function insertPatch(editor, patch, schema2) {
  ...
  if (patch.path.length > 1 && patch.path[1] !== "children")
    return debug$b("Ignoring patch targeting void value"), !1;
  if (patch.path.length === 1) {
    ...
    return debug$b(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), slate.Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
  ...
  return debug$b(`Inserting children at path ${childInsertPath}`), debugState(editor, "before"), childrenToInsert && slate.Element.isElement(childrenToInsert[0]) && slate.Transforms.insertNodes(editor, childrenToInsert[0].children, {
    at: childInsertPath
  }), debugState(editor, "after"), !0;
}
function setPatch(editor, patch) {
  ...
  if (debugState(editor, "before"), isTextBlock && child && childPath) {
    if (slate.Text.isText(value) && slate.Text.isText(child)) {
      ...
      debug$b("Setting non-text property"), editor.apply({
        type: "set_node",
        path: childPath,
        properties: {},
        newProperties: value
    ...
    debug$b("Setting block property");
    const {
      children,
      ...nextRest
    } = value, {
    ...
    }), debug$b("Setting children"), block.children.forEach((c, cIndex) => {
      editor.apply({
        type: "remove_node",
...
function unsetPatch(editor, patch) {
  if (patch.path.length === 0) {
    debug$b("Removing everything"), debugState(editor, "before");
  ...
  if (patch.path.length === 1) {
    ...
    return debug$b(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), slate.Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$b("Child not found"), !1) : (debug$b(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose && debug$b(`Removing child at path ${JSON.stringify(childPath)}`), slate.Transforms.removeNodes(editor, {
    at: childPath
...
function debugState(editor, stateName) {
  debugVerbose && (debug$b(`Children ${stateName}:`, JSON.stringify(editor.children, null, 2)), debug$b(`Selection ${stateName}: `, JSON.stringify(editor.selection, null, 2)));
}
    ...
    return editor.apply = (op) => {
      ...
      if (op.type === "remove_node") {
        ...
        if (op.path[0] === 0 && slate.Editor.isVoid(editor, node)) {
          ...
          editor.children[nextPath[0]] || (debug$9("Adding placeholder block"), slate.Editor.insertNode(editor, editor.pteCreateTextBlock({
            decorators: []
          })));
        }
      }
...
const debug$8 = debugWithName("plugin:withPortableTextBlockStyle");
      ...
      for (const op of editor.operations)
        if (op.type === "split_node" && op.path.length === 1 && editor.isTextBlock(op.properties) && op.properties.style !== defaultStyle && op.path[0] === path[0] && !slate.Path.equals(path, op.path)) {
          ...
          if (slate.Text.isText(child) && child.text === "") {
            debug$8(`Normalizing split node to ${defaultStyle} style`, op), editorActor.send({
              type: "normalizing"
            }), slate.Transforms.setNodes(editor, {
              style: defaultStyle
            }, {
...
const debug$4 = debugWithName("behaviors:event");
...
function performEvent({
  ...
  debug$4(`(${mode}:${eventCategory(event)})`, JSON.stringify(event, null, 2));
  ...
  if (eventBehaviors.length === 0 && isSyntheticBehaviorEvent(event)) {
    nativeEvent?.preventDefault(), withApplyingBehaviorOperations(editor, () => {
      debug$4(`(execute:${eventCategory(event)})`, JSON.stringify(event, null, 2)), performOperation({
        context: {
          keyGenerator,
          schema: schema2
        },
...
function RouteEventsToChanges(props) {
  ...
  $[2] !== handleChange || $[3] !== props.editorActor ? (t1 = () => {
    debug$2("Subscribing to editor changes");
    ...
    return () => {
      debug$2("Unsubscribing to changes"), sub.unsubscribe();
    };
